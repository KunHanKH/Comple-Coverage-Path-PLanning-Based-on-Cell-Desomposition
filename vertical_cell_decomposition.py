...
# Extract obstacles
# Obstacles have intersection among each other
obstacles = [];
...
for pt in new_sorted_vertices:
    ...
    # None represent that the current vertice is the end point
    # add the segment of each vertice
...
for index1 in range(len(open_line_segments) ):
    ...
    # done[0]: True if the lower pt is the current vertex
    # done[1]: True if the upper pt is the current vertex
    # done[2]: True if the there is a line througn the current vertex
    ...
    for index2 in range(index1+1,  len(open_line_segments)):
        ...
        # if there is no line through the vertex
        if( done[2] is False ):
            if(double_check):
    ...
    for index_cell2,cell in enumerate(quad_cells):
        if(index_cell != index_cell2):
            if(quad_cells[index_cell][0].x == cell[0].x and quad_cells[index_cell][1].x == cell[1].x):
                    ...
                    area1 = quad_polygon_area(temp1,4); 
                    area2 = quad_polygon_area(temp2,4);
                    new_quad=[];
                    ...
                    new_quad.append( point(temp1[0].x, min(temp1[0].y, temp2[0].y)) );
                    area3 = quad_polygon_area(new_quad, 4);
